    ...
    return (
        <div className="bg-dark-card text-white rounded-3xl p-8 md:p-10 shadow-2xl relative overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 border-b border-white/5 pb-8 relative z-10 gap-4">
                ...
                <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar pedido o cliente..." className="bg-white/5 border border-white/10 text-sm rounded-full px-6 py-3.5 w-full md:w-72 focus:outline-none focus:border-white transition-all placeholder:text-text-muted/50" />
            </div>

            <div className="flex flex-wrap gap-2 mb-8 relative z-10">
                {FILTERS.map((f) => (
                    <button key={f} onClick={() => setFilter(f)} className={`px-5 py-2.5 rounded-full text-[9px] font-black uppercase tracking-widest transition-all ${filter === f ? 'bg-white text-black' : 'bg-white/5 text-white/40 hover:text-white'}`}>{f}</button>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10">
                ...
                <div className="lg:col-span-5 space-y-3 max-h-[560px] overflow-y-auto pr-2 custom-scrollbar">
                    {loading ? (
                        <div className="py-20 flex justify-center"><div className="w-8 h-8 border-4 border-white/10 border-t-white rounded-full animate-spin" /></div>
                    ) : orders.length > 0 ? orders.map((order) => (
                        <div key={order.id} onClick={() => setSelected(order)} className={`p-5 rounded-2xl cursor-pointer flex justify-between items-center transition-all border ${selected?.id === order.id ? 'bg-dark-subcard border-white' : 'bg-white/5 border-transparent hover:bg-white/10'}`}>
                            <div>
                            ...
                            <div className="text-right">
                                <p className="text-lg font-black text-white tracking-tighter">${order.total}</p>
                        ...
                        <div className="flex flex-col h-full">
                            ...
                            <div className="bg-black/20 rounded-3xl p-5 mb-6 border border-white/5 flex-grow">
                                ...
                                <div className="space-y-2">
                                    {selected.items.map((it) => (
                                        <div key={it.id} className="flex justify-between text-sm border-b border-white/5 pb-2 last:border-0">
                                            ...
                                            <span className="text-white font-black">${it.precio_unitario * it.cantidad}</span>
                                        </div>
                                    ))}
                                </div>
                                <div className="flex justify-between pt-4 mt-2 border-t border-white/10">
                                    ...
                                    <span className="text-2xl font-black text-white">${selected.total}</span>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {STATES.map((s) => (
                                        <button key={s} disabled={s === selected.estado} onClick={() => changeStatus(selected, s)} className={`px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest transition-all ${s === selected.estado ? 'bg-white text-black cursor-default' : 'bg-white/5 text-white/50 hover:bg-white/10 hover:text-white'}`}>{s}</button>
                                    ))}
                                </div>
                            </div>
                        </div>
...
const badgeClass = (estado) => {
    if (estado === 'Enviado' || estado === 'Entregado') return 'bg-white/10 text-white border border-white/20';
